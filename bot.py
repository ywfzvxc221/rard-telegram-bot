...
def start(message):
    ...
    if not check_subscription(user_id):
        ...
        for channel in CHANNELS:
            markup.add(types.InlineKeyboardButton("اشترك هنا", url=channel))
        bot.send_message(message.chat.id, "مرحباً بك عزيزي! للاستخدام يجب الاشتراك في القنوات التالية:", reply_markup=markup)
        return
